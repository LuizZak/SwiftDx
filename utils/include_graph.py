...
def make_argparser() -> argparse.ArgumentParser:
    argparser = argparse.ArgumentParser(description='Generates a .dot Graphviz file with the include graph starting from a given header file. Results gets printed to stdout.')
    argparser.add_argument('header_file_path',
                           type=Path,
...
def run_cl(header_file_path: Path) -> str:
    ...
    process = subprocess.run(args, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
    process.check_returncode()

    return process.stderr.decode('UTF8')
...
def print_graphviz(graph: Graph) -> str:
    nodes = graphlib.TopologicalSorter(graph._graph).static_order()
    nodes = list(nodes)
    nodes: List[HeaderFile] = list(reversed(nodes))
...
def process(header_file_path: Path, output: str) -> str:
    ...
    def is_in_stack(header: HeaderFile) -> bool:
        for item in stack:
            if item.path.samefile(header.path):
                return True
