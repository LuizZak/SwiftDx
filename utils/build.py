...
from typing import Any, Callable, T, List, Optional
...
def run_output(bin_name: str, *args: List[str], echo: bool = True) -> bytes:
    if echo:
        print('>', bin_name, *list(args))
    
    return subprocess.check_output([bin_name] + list(args))

def run(bin_name: str, *args: List[str], echo: bool = True, silent: bool = False):
    if echo:
        print('>', bin_name, *list(args))
...
def main() -> int:
    ...
    match args.command:
        case 'build': do_build_command(args)
        case 'test': do_test_command(args)
